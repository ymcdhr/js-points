...
如果在客户端APP中webview打开wap页面，需要做一些操作时，会由客户端提供js方法供wap页调用：
1、客户端会暴露js接口给wap页面使用，wap中相当于直接调用全局方法。
2、在wap调用客户端方法是要注意：客户端暴露的接口不一定在wap页加载完成时就能用（有可能客户端并没有加载完成）。如果不能使用，需要定时去查询客户端方法，直到加载完成后才能在wap端调用。
