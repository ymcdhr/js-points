# 本页目录
[1、公共的HTML头部](#公共的html头部)
[2、使用arttemplate渲染模版（包括子模板的渲染）](#使用arttemplate渲染模版)
[3、使用zepto代替jquery作为库文件](#使用zepto代替jquery作为库文件)
[4、使用rem单位来做响应式布局，并规范设计尺寸](#使用rem单位来做响应式布局，并规范设计尺寸)
[5、页面中的跨域方案，通常使用jsonp，但也可以使用cors方式](#页面中的跨域方案，通常使用jsonp，但也可以使用cors方式)
[6、微信分享接口的调用](#微信分享接口的调用)
[7、wap端页面的调试，chrome/远程/app调试](#wap端页面的调试，chrome/远程/app调试)
[8、唤起客户端的方法](#唤起客户端的方法)
[9、页面的埋点方案](#页面的埋点方案)
[10、页面数据分页加载的方案](#页面数据分页加载的方案)
[11、页面图片或者dom懒加载的方案](#页面图片或者dom懒加载的方案)
[12、页面性能优化方案](#页面性能优化方案)
